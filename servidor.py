...
1. SALUDO INICIAL: SÓLO EN EL PRIMER MENSAJE DE LA CONVERSACIÓN debes decir OBLIGATORIA Y EXACTAMENTE esto: "Hola, bienvenido a tu asesor de WoodTools.". Si el cliente NO dijo qué busca, completa preguntando: "¿Cuál es la herramienta que estás buscando?". Si YA LO DIJO, saluda y pasa a la siguiente pregunta. ¡NUNCA REPITAS EL SALUDO DE BIENVENIDA EN MENSAJES POSTERIORES!
...
def procesar_mensaje_con_gemini(telefono_cliente, texto_entrante, imagen_pil=None):
    ...
    if resultado:
        ...
        historial = [
            ...
            {"role": "model", "parts": ["Entendido. Guardaré en mi memoria todos los servicios adicionales y respetaré la brevedad absoluta. NUNCA volveré a pedir el asesor si ya envié el link de derivación y seré inteligente con las imágenes."]}
        ]
    ...
    try:
        ...
        if imagen_pil:
            param_vision = "INSTRUCCIÓN DE VISIÓN (No le digas esto al cliente): Analiza esta imagen de una madera con un corte o ensamble. Distingue OBLIGATORIAMENTE entre estas dos: 1) Fresa para Ensamble Cónico HM: La madera unida muestra dientes GRANDES, GRUESOS, y con las PUNTAS PLANAS o RECTAS (como bloques o trapecios chatos en la punta). Se ven pocos dientes (generalmente 3 o 4 canales grandes). 2) Fresa para Finger HM: La madera unida muestra MUCHOS dientes PEQUEÑOS, PUNTIAGUDOS y en forma de 'V' afilada (zig-zag perfecto sin puntas chatas). Identifica la herramienta y responde al cliente basándote en ello, continuando la indagación de forma natural."
