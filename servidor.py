...
1. SALUDO INICIAL: Tu primer mensaje de la conversación debe empezar OBLIGATORIAMENTE con: "Hola, bienvenido a tu asesor de WoodTools.". Si el cliente en su primer mensaje NO dijo qué herramienta busca, completa el saludo preguntando: "¿Cuál es la herramienta que estás buscando?". PERO SI EL CLIENTE YA DIJO QUÉ HERRAMIENTA BUSCA (ej: "quiero fresas"), NO le preguntes qué herramienta busca; salúdalo y pasa directamente a la siguiente pregunta (ej: qué tipo de fresa, o elección de asesor).
...
def procesar_mensaje_con_gemini(telefono_cliente, texto_entrante, imagen_pil=None):
    ...
    try:
        ...
        if imagen_pil:
            param_vision = "INSTRUCCIÓN DE VISIÓN (No le digas esto al cliente): Analiza esta imagen de una herramienta. Si es una madera con un corte o ensamble, distingue cuidadosamente: 1) Fresa para Ensamble Cónico HM: Deja dientes más gruesos, rectos o ligeramente en diagonal (cónicos) con puntas chatas/planas. Generalmente se ven 3 o 4 canales grandes como dedos cuadrados en la madera. 2) Fresa para Finger HM: Deja muchos dientes pequeñitos, puntiagudos y en forma de 'V' perfecta (zig-zag). Identifica la herramienta correspondiente y responde al cliente basándote en ello, continuando la indagación de forma natural."
