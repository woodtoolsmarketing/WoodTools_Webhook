...
def obtener_prompt_personalizado(telefono_cliente_completo):
    ...
    if not es_organico:
        ...
        texto_contexto = """CONTEXTO: Cliente "Orgánico". Sin vendedor asignado. PREGUNTA OBLIGATORIAMENTE con qué asesor prefiere hablar (Carlos, Valentín o Emmanuel). ¡ATENCIÓN!: Si el cliente responde "me da igual", "cualquiera", "no los conozco" o "no sé" a la elección del asesor, TIENES ESTRICTAMENTE PROHIBIDO volver a preguntarle. ASIGNA TÚ INMEDIATAMENTE uno al azar (Ej: Carlos, Valentín o Emmanuel) y avanza con la consulta técnica."""
...
1. SALUDO INICIAL: Tu primer mensaje de la conversación debe empezar con: "Hola, bienvenido a tu asesor de WoodTools.". REGLA DE ORO: LEE EL HISTORIAL. Si ya dijiste este saludo anteriormente, TIENES ABSOLUTAMENTE PROHIBIDO volver a decirlo en toda la conversación. Solo se saluda UNA VEZ.
...
5. (Solo si es orgánico): PREGUNTAR OBLIGATORIAMENTE qué asesor prefiere (Carlos, Valentín o Emmanuel). Si elije o le da igual, guárdalo y no vuelvas a preguntar.
...
3. ADVERTENCIA CRÍTICA CON EL ENLACE: Presta muchísima atención a la estructura de la URL. NO ELIMINES ninguna barra (/) ni el número de teléfono del cliente que ya está incluido en la plantilla (que dice {tel_10_digitos}). 
...
def procesar_mensaje_con_gemini(telefono_cliente, texto_entrante, imagen_pil=None):
    ...
    if resultado:
        ...
        historial = [
            ...
            {"role": "model", "parts": ["Entendido. Guardaré en mi memoria todos los servicios adicionales y respetaré la brevedad absoluta. NUNCA repetiré el saludo inicial y si el cliente dice que le da igual el asesor, elegiré uno al azar inmediatamente sin volver a preguntarlo."]}
        ]
    ...
    try:
        ...
        if imagen_pil:
            param_vision = "INSTRUCCIÓN DE VISIÓN CRÍTICA (NO se lo leas al cliente): Analiza esta foto de madera. TU OBJETIVO ES NO CONFUNDIRTE. 1) Si ves dientes GRANDES, GRUESOS, y con PUNTAS TOTALMENTE CHATAS O PLANAS (como bloques rectangulares o trapecios chatos en la punta), es ABSOLUTAMENTE una 'Fresa para Ensamble Cónico HM' (JAMÁS le digas Finger o Medio círculo). 2) Si ves MUCHOS dientes PEQUEÑITOS, continuos en zig-zag y con PUNTAS AFILADAS en forma de 'V', es 'Fresa para Finger HM'. Identifica correctamente. Dile al cliente exactamente qué herramienta es y continúa con la indagación (ej: pedir espesor o profundidad)."
